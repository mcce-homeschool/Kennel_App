...
**What is built vs. designed-only:** the schema, entities, and rules below are live in code through Stage 4.5 **except** the test-planning fields (`Dog.planned_tests`, `Kennel.preferred_tests`) and everything that hangs off them (§5.11, the §5.2 test-name vocabulary note, the §14 authoring surfaces). Those are a **settled design that is not yet built** — flagged inline wherever they appear.
...
| planned_tests | string[] | | **Designed, not yet built (§5.11).** Unindexed. This dog's intended health/genetic tests. Plain strings, not FKs — never enter the reference registry. |
...
> **Test-name strings as vocabulary — designed, not yet built (§5.11).** The plan is for the test-name `details` fields on `genetic_test` (`panel_name`), `breed_specific_test` (`test_name`), and `ofa_pennhip` to be the "seen-in-events" half of a shared suggestion vocabulary, unioned with `Kennel.preferred_tests`. This lands with the test-planning feature; it is not in the current build.
...
| preferred_tests | string[] | | **Designed, not yet built (§5.11).** Unindexed. The kennel's authored health-test panel and shared suggestion vocabulary. |
| notes | text | | |
| is_archived | boolean | ✓ | |

### 5.11 Test-planning fields — designed, not yet built
*This is a settled design that has **not** shipped in the current build (no `planned_tests` / `preferred_tests` in code as of Stage 4.5). It is Stage-5-adjacent and can land independently — nothing about it changes the schema version, the reference registry, or the JSON backup format. Kept here so the design is ready to build against; the behavioral spec lives in the Test Planning & Vocabulary addendum.*
...
- The exporter **iterates whatever tables exist in the schema at that point**, so it stays correct as additive `.version(N)` blocks are added after the first real release, with no code changes. Plain fields — `Event.event_end_date`, `Event.related_contact_id`, and (once built) `Dog.planned_tests` / `Kennel.preferred_tests` — ride along on their already-exported records for free.
...
- **`Dog.planned_tests` / `Kennel.preferred_tests` are strings, not references** (when built) — they never appear in any registry and never block a delete.
...
- **Test authoring vs. event capture are separate surfaces (designed, not yet built).** When test-planning lands, planning happens at the kennel level (authored panel) and dog level (curation); the event screen stays a lean suggest-combobox and never presents the panel as a checklist to reconcile mid-task.
...
**Not yet built:** the test-planning feature (§5.11 and its dependents) and all of Stage 5 (dashboards, advanced breeder tools, COI/genetic analysis, reminder engine). No further schema revision is expected before Stage 5; when the first real release ships, additive `.version(2)` blocks begin.
