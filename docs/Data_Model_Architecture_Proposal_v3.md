...
- **Seed import restored** (addendum §8–9; additive, no version bump, no reference-registry/backup-format change) — the optional breed+test prefill, originally deferred out of the shipped scope, was added: new plain unindexed `Kennel.preferred_breeds` (§5.11) with `kennelRepo.addPreferredBreed`/`getBreedVocabulary`, unioned into the dog-form breed autocomplete alongside `dogRepo.getBreeds` (so a seeded breed suggests before the first dog exists); a dedicated `pages/kennel-tests-import.{html,js}` view (linked from Import/Export) that parses a `breed, test_name` CSV with per-breed checkboxes + dry-run preview and appends to `preferred_tests`/`preferred_breeds` on the target own-kennel — deliberately *not* through the generic `csvImport.js` record engine; and the shipped `KennelOS/resources/common_tests_by_breed_seed.csv` starter file with its illustrative/verify-against-source disclaimer.
```
