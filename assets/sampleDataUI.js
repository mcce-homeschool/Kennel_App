...
export async function promptClearSampleData() {
  ...
  if (!window.confirm('Remove all sample data (Thornfield Kennels demo dogs, contacts, kennels, and events)? This cannot be undone.')) {
    return;
  }
  ...
  if (result.cleared) {
    const parts = [`${result.counts.dogs} dog(s)`, `${result.counts.events} event(s)`,
