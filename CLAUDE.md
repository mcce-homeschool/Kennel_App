...
The optional **seed import** (Test Planning addendum §8–9) is built: a shipped `breed, test_name` CSV (`KennelOS/resources/common_tests_by_breed_seed.csv`) imports through a dedicated view (`pages/kennel-tests-import.*`, linked from Import/Export) that appends to `Kennel.preferred_tests` and the new plain `Kennel.preferred_breeds` pool (which feeds breed autocomplete before any dog exists). It is deliberately NOT routed through the generic `csvImport.js` record engine — vocabulary-append is a different shape. Still zero-migration, still no reference-registry/backup-format change.
