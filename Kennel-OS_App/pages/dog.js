...
const blankDog = () => ({
  ...
  co_owner_contact_ids: [], litter_id: '', status: '', status_date: '', notes: ''
});
...
async function loadRefs() {
  const [dogs, contacts, litters, breeds] = await Promise.all([
...
function renderEdit() {
  ...
  // kennel-setup contact, if one is set and Owner is still empty. Only a
  // prefill — never overrides a value already chosen.
  document.getElementById('f-ownership_type').addEventListener('change', (e) => {
    if (e.target.value !== 'owned') return;
    ctx.draft = readForm();
    if (ctx.draft.owner_contact_id) return;
    const myContactId = getMyContactId();
    if (!myContactId || !ctx.contactsById.has(myContactId)) return;
    ctx.draft.owner_contact_id = myContactId;
    renderEdit();
  });
  updateWarnings();
}
