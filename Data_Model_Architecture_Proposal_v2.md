...
- **Descendant lookup:** the reverse direction requires an index, since Dexie doesn't do this natively — maintain an in-memory lookup map (`parent_id → [child_ids]`) built once per session from the Dog table, refreshed on write. Cheap at kennel-scale record counts (hundreds to low thousands of dogs).
...
The remaining entity-specific mappings (Litter, Buyer, Sale, StudService, Contact) follow this same match-or-create pattern and can be finalized during Stage 1 build-out — the mechanism is fixed now, the column lists are a small, low-risk detail to nail down per epic.
...
- The exporter **iterates whatever tables exist in the schema at that point**, so it's correct with the four Stage 1–2 tables and with all ten later without code changes.
...
// polymorphic Event) the discriminator that must also match.
export const DOG_REFERENCES = [
  ...
  { table: 'events', field: 'subject_id',     label: 'subject of an event', whereType: { subject_type: 'dog' } },
  { table: 'events', field: 'related_dog_id', label: 'partner on an event' },
  // Stage 3+ append: pairings.sire_id, pairings.dam_id
  // Stage 4+ append: litters.sire_id, litters.dam_id
  // Stage 6+ append: sales.dog_id, stud_services.our_dog_id, stud_services.partner_dog_id
];
```

- `canHardDelete(dogId)` iterates `DOG_REFERENCES`, **skipping any entry whose table isn't in the current schema version**, and returns the list of human-readable blockers. At Stage 2 that means it genuinely checks only `dogs` (sire/dam) and `events` (both prongs) — and says so — rather than claiming to check litters/sales/stud-services that don't exist yet.
...
- **Pedigree view should be spatial, not a list** — a family-tree chart, not a table. Each node is a dog; clicking a node's name navigates to / re-centers on that dog. This requires no schema change — `Dog.sire_id`/`dam_id` already form the graph the chart would render. Implementation note for later: a small client-side charting library (e.g. D3.js tree layout), **vendored into the repo like Dexie** (not CDN), keeps this fully static and offline-capable. Dogs with unknown/missing parents render as a visible placeholder node rather than silently truncating the branch.

## 15. Suggested Next Step

Stage 1 (Foundation) can start directly from §5, §10, and §11 above: define the Dexie schema, build the repo layer and reference registry, and stand up the import/export framework against these ten tables. Per-epic screen mockups and acceptance criteria can be scoped stage-by-stage against this model without needing to revisit the schema itself.
